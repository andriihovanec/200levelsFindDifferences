...
import com.olbigames.levels200finddifferences.SQLiteHelper.HiddenHintData;
...
public class GameRenderer implements Renderer {
  ...
  //+++++++++++++++++++++++++++++++++++++++++++++++++++++++
	...
	public GameRenderer(Context c, int width, int height, int bh, SQLiteHelper db0, int lvl, float vl)
	{
		...
		/*
		String extens = GLES20.glGetString(GL10.GL_EXTENSIONS);
		mSupportNPOT = extens.indexOf("GL_OES_texture_npot") != -1;
		...
		*/
		...
		if(hhd.f == 0.0f){
		    ...
		    hintY = hhd.y;
		    hintSize = hhd.r;
		    //showHiddenHint = !hiddenHint.finded;
		}
	    //-------HIDDEN HINT END------
		level = lvl;
        ...
        //sselect = sounds.load(mContext, R.raw.select, 1);
        //sconfirm = sounds.load(mContext, R.raw.confirm, 1);
        sbeep = sounds.load(mContext, R.raw.beep, 1);
        /*
		sounds.setVolume(sselect, volumeLevel, volumeLevel);
		sounds.setVolume(sconfirm, volumeLevel, volumeLevel);
		sounds.setVolume(sbeep, volumeLevel, volumeLevel);
		*/
	}
	...
	public void onSurfaceCreated(GL10 gl, EGLConfig config) {
		...
		Log.e("GL", ": GL_MAX_TEXTURE_SIZE " + maxSize[0]);
		*/
        ...
        //vColor = GLES20.glGetUniformLocation (riGraphicTools.sp_Point, "vColor" );
		...
		Bitmap bmp = MyExpansionHelper.loadPic(mContext, level - 1, 0, 1);
		...
		//id = mContext.getResources().getIdentifier("raw/pic"+String.format("%04d",level)+"_2", null, mContext.getPackageName());
		...
		Bitmap bmp2 = MyExpansionHelper.loadPic(mContext, level - 1, 0, 2);
		rect2 = new RectangleImage( vertices2, bmp2, 1);
		//bmp.recycle();
		
		/*
		vertices = new float[]
		           {mScreenWidth - menuButWidth, menuButHeight + 20f, 0.0f,
					mScreenWidth - menuButWidth, 20.0f, 0.0f,
					mScreenWidth, 20.0f, 0.0f,
					mScreenWidth, menuButHeight + 20f, 0.0f
		           };
		id = mContext.getResources().getIdentifier("raw/buttons", null, mContext.getPackageName());
		bmp = BitmapFactory.decodeResource(mContext.getResources(), id);
		rect3 = new RectangleImage( vertices, bmp, 2);
		bmp.recycle();
		*/
...
/*
id = mContext.getResources().getIdentifier("raw/pic"+String.format("%04d",level)+"_1", null, mContext.getPackageName());
bmp = BitmapFactory.decodeResource(mContext.getResources(), id, dimensions);
*/
...
/*
	public void setVerticles(){
		
		//rect3.squareCoords = differences.getVert() ;
...
*/
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		...
		mScreenHeight = height;
		/*
		if( mScreenWidth / mScreenHeight < 1.77777f){
			
			float yOtstup = mScreenHeight - mScreenWidth/1.77777f;
			
		}else{
			
			float xOtstup = mScreenWidth - mScreenHeight*1.77777f;
	...
	public void onDrawFrame(GL10 unused) {
        ...
        if( showHiddenHint ) {
            if(ishiddenHintAnimShowing)
            {
                ...
                {
                    //Log.e("hiddenHintFounded", "showHiddenHint = false;");
                    showHiddenHint = false;
                    plus_one.start();
                    //[self plusOneAnim];
                }
...
//----Draw Hidden Hint End---------
	...
	public void Touched ( float x, float y){
		...
		if(xx != -1f & rect1!=null){
	        ...
	        //Log.e("FFFFF", "setDifferences :"+ xx + ":" + yy);
			...
			if (result != -1){
	            ...
	            if(showHiddenHint){
	                ...
	                if( hintX - hintSize < xx && xx < hintX + hintSize && hintY - hintSize  < yy && yy < hintY + hintSize ){
	                    ...
	                    //hiddenHint.setScale(rect1.getScale());
	                    
	                    //hiddenHint = new HiddenHint(mScreenWidth - 1.5f*banner_height, mScreenHeight - banner_height, hintSize, 0, gx, gy);
	        			...
	        			//hiddenHint = [[HiddenHint alloc] init];
	                    //CGRect r = btn3.frame;
	                    //[hiddenHint set:r.origin.x + r.size.width/2 finalY: _height - r.origin.y xx:gx yy:gy scale: _picScale /_scale];
	                    //[self hiddenHintFounded];
        ...
        /*
		Traces trace = new Traces ( x, mScreenHeight - y);
		traces.add(trace);
	...
	public void doMove ( float x, float y){
		if(rect1!=null){rect1.translate(x,y);}
		if(rect2!=null){rect2.translate(x,y);}
	}

	public void doScale ( float x){

		if(rect1!=null){rect1.scale(x);}
		if(rect2!=null){rect2.scale(x);}
		/*
		if(x > mScreenWidth/2){// Приближаем
			rect1.scale(-5f);
			rect2.scale(-5f);
		}else{// Отдаляем
			rect1.scale(5f);
